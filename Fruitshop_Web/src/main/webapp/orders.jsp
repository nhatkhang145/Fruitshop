      ...
      <div class="profile-container">
        ...
        <main class="profile-content orders-content">
          ...
          <c:if test="${not empty sessionScope.successMessage}">
            <div class="alert alert-success" style="background: #d4edda; color: #155724; padding: 12px; margin-bottom: 15px; border-radius: 5px;">
          ...
          <c:if test="${not empty sessionScope.errorMessage}">
            <div class="alert alert-danger" style="background: #f8d7da; color: #721c24; padding: 12px; margin-bottom: 15px; border-radius: 5px;">
            ...
            <c:choose>
              ...
              <c:otherwise>
                <c:forEach items="${orders}" var="order">
                  <div class="order-card" style="border: 1px solid #ddd; border-radius: 8px; padding: 15px; margin-bottom: 15px;">
                    <div class="order-card__header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px; padding-bottom: 10px; border-bottom: 1px solid #eee;">
                      ...
                      <span class="order-status status-${order.status}" style="padding: 5px 15px; border-radius: 20px; background: #f0f0f0; font-size: 13px;">
                        ${order.statusDisplay}
                      </span>
                    </div>
                    ...
                    <div class="order-card__footer" style="display: flex; justify-content: space-between; align-items: center;">
                      <div class="order-total">
                        ...
                        <strong class="price" style="color: var(--primary-color); font-size: 18px; margin-left: 10px;">
                      ...
                      <div class="order-actions" style="display: flex; gap: 10px;">
                        <a href="${pageContext.request.contextPath}/order-detail?id=${order.id}" 
                           class="btn btn-outline" style="padding: 8px 20px; border: 1px solid var(--primary-color); color: var(--primary-color); border-radius: 5px; text-decoration: none;">
                          Xem chi tiết
                        </a>
                        <c:if test="${order.status == 'pending' || order.status == 'processing'}">
                          <form action="${pageContext.request.contextPath}/orders" method="post" 
                            ...
                            <button type="submit" class="btn btn-danger" style="padding: 8px 20px; background: #dc3545; color: #fff; border: none; border-radius: 5px; cursor: pointer;">Hủy đơn</button>
                          </form>
                        </c:if>
                        <c:if test="${order.status == 'completed'}">
                          <a href="${pageContext.request.contextPath}/shop" class="btn btn-primary" style="padding: 8px 20px; background: var(--primary-color); color: #fff; border-radius: 5px; text-decoration: none;">
                            Mua lại
                          </a>
                        </c:if>
                      </div>
